// <auto-generated />
using System;
using FM_API.Persistance.Database;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace FM_API.Migrations
{
    [DbContext(typeof(FMContext))]
    [Migration("20220509135104_FourteenthMigration")]
    partial class FourteenthMigration
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasDefaultSchema("public")
                .HasAnnotation("ProductVersion", "6.0.3")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("FM_API.Entities.Budget", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("Create_at")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("Deleted_at")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("Month")
                        .HasColumnType("integer");

                    b.Property<DateTime?>("Update_at")
                        .HasColumnType("timestamp with time zone");

                    b.Property<long>("Year")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.ToTable("presupuesto", "public");
                });

            modelBuilder.Entity("FM_API.Entities.Estimate", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("Id_budget")
                        .HasColumnType("bigint");

                    b.Property<long>("Id_category")
                        .HasColumnType("bigint");

                    b.Property<int>("Plan")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("Id_budget");

                    b.HasIndex("Id_category");

                    b.ToTable("estimaciones", "public");
                });

            modelBuilder.Entity("FM_API.Entities.Income", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<int>("Amount")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.ToTable("ingresos", "public");
                });

            modelBuilder.Entity("FM_API.Entities.Rol", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Rol_type")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("rol", "public");
                });

            modelBuilder.Entity("FM_API.Entities.Spent", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<int>("Amount")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.ToTable("gastos", "public");
                });

            modelBuilder.Entity("FM_API.Entities.Transaction", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<int>("Amount")
                        .HasColumnType("integer");

                    b.Property<string>("Description")
                        .HasColumnType("text");

                    b.Property<long>("Id_budget")
                        .HasColumnType("bigint");

                    b.Property<long>("Id_category")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.HasIndex("Id_budget");

                    b.HasIndex("Id_category");

                    b.ToTable("transacciones", "public");
                });

            modelBuilder.Entity("FM_API.Entities.User", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<long>("Id_rol")
                        .HasColumnType("bigint");

                    b.Property<string>("Lastname")
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .HasColumnType("text");

                    b.Property<string>("Pass")
                        .IsRequired()
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("Id_rol");

                    b.ToTable("usuario", "public");
                });

            modelBuilder.Entity("FMAPI.Entities.Category", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<DateTime>("Create_at")
                        .HasColumnType("timestamp with time zone");

                    b.Property<DateTime?>("Deleted_at")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Description")
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<DateTime?>("Update_at")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("Id");

                    b.ToTable("categoria", "public");
                });

            modelBuilder.Entity("FMAPI.Entities.EstimateIncome", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("Id_Estimate")
                        .HasColumnType("bigint");

                    b.Property<long>("Id_Income")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.HasIndex("Id_Estimate");

                    b.HasIndex("Id_Income");

                    b.ToTable("estimate_income", "public");
                });

            modelBuilder.Entity("FMAPI.Entities.EstimateSpent", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("Id_Estimate")
                        .HasColumnType("bigint");

                    b.Property<long>("Id_Spent")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.HasIndex("Id_Estimate");

                    b.HasIndex("Id_Spent");

                    b.ToTable("estimate_spent", "public");
                });

            modelBuilder.Entity("FMAPI.Entities.TransactionIncome", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("Id_Income")
                        .HasColumnType("bigint");

                    b.Property<long>("Id_Transaction")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.HasIndex("Id_Income");

                    b.HasIndex("Id_Transaction");

                    b.ToTable("TransactionIncome", "public");
                });

            modelBuilder.Entity("FMAPI.Entities.TransactionSpent", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<long>("Id"));

                    b.Property<long>("Id_Spent")
                        .HasColumnType("bigint");

                    b.Property<long>("Id_Transaction")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.HasIndex("Id_Spent");

                    b.HasIndex("Id_Transaction");

                    b.ToTable("TransactionSpent", "public");
                });

            modelBuilder.Entity("FM_API.Entities.Estimate", b =>
                {
                    b.HasOne("FM_API.Entities.Budget", "Budget")
                        .WithMany("Estimates")
                        .HasForeignKey("Id_budget")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("FMAPI.Entities.Category", "Category")
                        .WithMany()
                        .HasForeignKey("Id_category")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Budget");

                    b.Navigation("Category");
                });

            modelBuilder.Entity("FM_API.Entities.Transaction", b =>
                {
                    b.HasOne("FM_API.Entities.Budget", "Budget")
                        .WithMany("Transactions")
                        .HasForeignKey("Id_budget")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("FMAPI.Entities.Category", "Category")
                        .WithMany()
                        .HasForeignKey("Id_category")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Budget");

                    b.Navigation("Category");
                });

            modelBuilder.Entity("FM_API.Entities.User", b =>
                {
                    b.HasOne("FM_API.Entities.Rol", "Rol")
                        .WithMany()
                        .HasForeignKey("Id_rol")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Rol");
                });

            modelBuilder.Entity("FMAPI.Entities.EstimateIncome", b =>
                {
                    b.HasOne("FM_API.Entities.Estimate", "Estimate")
                        .WithMany("Income")
                        .HasForeignKey("Id_Estimate")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("FM_API.Entities.Income", "Income")
                        .WithMany()
                        .HasForeignKey("Id_Income")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Estimate");

                    b.Navigation("Income");
                });

            modelBuilder.Entity("FMAPI.Entities.EstimateSpent", b =>
                {
                    b.HasOne("FM_API.Entities.Estimate", "Estimate")
                        .WithMany("Expenses")
                        .HasForeignKey("Id_Estimate")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("FM_API.Entities.Spent", "Spent")
                        .WithMany()
                        .HasForeignKey("Id_Spent")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Estimate");

                    b.Navigation("Spent");
                });

            modelBuilder.Entity("FMAPI.Entities.TransactionIncome", b =>
                {
                    b.HasOne("FM_API.Entities.Income", "Income")
                        .WithMany()
                        .HasForeignKey("Id_Income")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("FM_API.Entities.Transaction", "Transaction")
                        .WithMany("Income")
                        .HasForeignKey("Id_Transaction")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Income");

                    b.Navigation("Transaction");
                });

            modelBuilder.Entity("FMAPI.Entities.TransactionSpent", b =>
                {
                    b.HasOne("FM_API.Entities.Spent", "Spent")
                        .WithMany()
                        .HasForeignKey("Id_Spent")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("FM_API.Entities.Transaction", "Transaction")
                        .WithMany("Expenses")
                        .HasForeignKey("Id_Transaction")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Spent");

                    b.Navigation("Transaction");
                });

            modelBuilder.Entity("FM_API.Entities.Budget", b =>
                {
                    b.Navigation("Estimates");

                    b.Navigation("Transactions");
                });

            modelBuilder.Entity("FM_API.Entities.Estimate", b =>
                {
                    b.Navigation("Expenses");

                    b.Navigation("Income");
                });

            modelBuilder.Entity("FM_API.Entities.Transaction", b =>
                {
                    b.Navigation("Expenses");

                    b.Navigation("Income");
                });
#pragma warning restore 612, 618
        }
    }
}
